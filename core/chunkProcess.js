...
async function renderChunk() {
  try {
    ...
    // 重建timeline对象
    const timeline = new ProcessTimeline(serializedTimeline);
    
    // 重建tracks
    for (const [trackId, trackData] of timeline.tracks) {
      const track = new ProcessTrack(trackData.type, trackData.config);
    ...
    for (let frameIndex = startFrame; frameIndex < endFrame; frameIndex++) {
      ...
      if (options.verbose) {
        console.log(`子进程 ${chunkIndex}: 渲染帧 ${frameIndex}/${endFrame} (${currentTime.toFixed(2)}s)`);
      }
      ...
      if (!frameData) {
        console.warn(`子进程 ${chunkIndex}: 时间 ${currentTime}s 无帧数据`);
        continue;
      }
      ...
      if (framesWritten % 10 === 0) {
        ...
        console.log(`子进程 ${chunkIndex} 进度: ${progress}%`);
      }
    }
