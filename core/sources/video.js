...
export default defineFrameSource("video", async (options) => {
    ...
    let inputCodec;
    if (firstVideoStream?.codec_name === "vp8")
        inputCodec = "libvpx";
    else if (firstVideoStream?.codec_name === "vp9")
    ...
    ps.catch((err) => {
        if (!err.isCanceled)
            throw err;
        if (verbose)
            console.log("ffmpeg process aborted", path);
    });
    ...
    async function readNextFrame(progress, canvas, time) {
        ...
        if (done) {
            if (verbose)
                console.log(path, "ffmpeg video stream ended");
            return;
        }
        if (!rgba) {
            if (verbose)
                console.log(path, "No frame data received");
            return;
        }
        if (logTimes)
