...
async function Editly(config) {
	...
	if (verbose) console.log('Calculated', JSON5.stringify({
		clips,
		arbitraryAudio
	},
	null, 2));
	...
	0);
	const {
		runTransitionOnFrame: runGlTransitionOnFrame
	} = GlTransitions({
		width,
		height,
		channels
	...
	function getOutputArgs() {
		...
		const videoOutputArgs = isGif ? ['-vf', `format = rgb24, fps = $ {
			fps
		},
		scale = $ {
			width
			...
			height
		}: flags = lanczos, split[s0][s1]; [s0] palettegen[p]; [s1][p] paletteuse`, '-loop', '0', ] : ['-vf', 'format=yuv420p', '-vcodec', 'libx264', '-profile:v', 'high', ... (fast ? ['-preset:v', 'ultrafast'] : ['-preset:v', 'medium']), '-crf', '18', '-movflags', 'faststart', ];
	...
	function startFfmpegWriterProcess() {
		const args = [... (enableFfmpegLog ? [] : ['-hide_banner', '-loglevel', 'error']), '-f', 'rawvideo', '-vcodec', 'rawvideo', '-pix_fmt', 'rgba', '-s', `$ {
			width
		}
		x$ {
			height
		}`, '-r', framerateStr, '-i', '-', ... (audioFilePath ? ['-i', audioFilePath] : []), ... (!isGif ? ['-map', '0:v:0'] : []), ... (audioFilePath ? ['-map', '1:a:0'] : []), ...getOutputArgs(), '-y', outPath, ];
		...
		try {
			...
			while (!outProcessError) {
				...
				if (!verbose) {
					...
					if (totalFramesWritten % 10 === 0) process.stdout.write(`$ {
				...
				if (transitionFrameAt >= transitionLastFrameIndex) {
					...
					console.log(`Done with transition, switching to next transitionFromClip($ {
				...
				if (verbose) {
					if (isInTransition) console.log('Writing frame:', totalFramesWritten, 'from clip', transitionFromClipId, ` (frame $ {
						fromClipFrameAt
					})`, 'to clip', getTransitionToClipId(), ` (frame $ {
						toClipFrameAt
					}
					/ ${transitionNumFramesSafe})`, currentTransition.name, `${currentTransition.duration}s`);
                    else
				...
				if (outProcessError) break; totalFramesWritten += 1; fromClipFrameAt += 1;
