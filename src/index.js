...
async function Editly(config) {
    const { 
    // Testing options:
    enableFfmpegLog = false, verbose = false, logTimes = false, keepTmp = false, fast = false, outPath, clips: clipsIn, clipsAudioVolume = 1, audioTracks: arbitraryAudioIn = [], width: requestedWidth, height: requestedHeight, fps: requestedFps, defaults = {}, audioFilePath: backgroundAudioPath, backgroundAudioVolume, loopAudio, keepSourceAudio, allowRemoteRequests, audioNorm, outputVolume, customOutputArgs, ffmpegPath = 'ffmpeg', ffprobePath = 'ffprobe', } = config;
    ...
    if (verbose)
        console.log('Calculated', JSON5.stringify({ clips, arbitraryAudio }, null, 2));
    ...
    try {
            ...
            while (!outProcessError) {
                ...
                if (isInTransition) {
                    ...
                    if (frameSource2Data) {
                        ...
                        if (logTimes)
                            console.timeEnd('runTransitionOnFrame');
                    }
                    else {
                ...
                if (verbose) {
                    if (isInTransition)
                        console.log('Writing frame:', totalFramesWritten, 'from clip', transitionFromClipId, `(frame ${fromClipFrameAt})`, 'to clip', getTransitionToClipId(), `(frame ${toClipFrameAt} / ${transitionNumFramesSafe})`, currentTransition.name, `${currentTransition.duration}s`);
                    else
                        console.log('Writing frame:', totalFramesWritten, 'from clip', transitionFromClipId, `(frame ${fromClipFrameAt})`);
                    // console.log(outFrameData.length / 1e6, 'MB');
                }
                const nullOutput = false;
                if (logTimes)
                    console.time('outProcess.write');
                ...
                if (logTimes)
                    console.timeEnd('outProcess.write');
                if (outProcessError)
                    break;
        ...
        try {
            ...
            if (outProcessExitCode !== 0 && !err.killed)
                throw err;
        }
    }
    finally {
 ...
 * @param config - Config.
 */
export async function renderSingleFrame(config) {
    const { time = 0, defaults = {}, width = 800, height = 600, clips: clipsIn, verbose, logTimes, enableFfmpegLog, allowRemoteRequests, ffprobePath = 'ffprobe', ffmpegPath = 'ffmpeg', outPath = `${Math.floor(Math.random() * 1e12)}.png`, } = config;
