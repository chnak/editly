...
async function Editly(input) {
    ...
    const { 
    // Testing options:
    verbose = false, logTimes = false, keepTmp = false, fast = false, outPath, clips: clipsIn, clipsAudioVolume, audioTracks: arbitraryAudioIn, width: requestedWidth, height: requestedHeight, fps: requestedFps, audioFilePath: backgroundAudioPath, backgroundAudioVolume, loopAudio, keepSourceAudio, allowRemoteRequests, audioNorm, outputVolume, customOutputArgs, isGif, tmpDir, defaults, } = config;
    ...
    const estimatedTotalFrames = fps *
    ...
    try {
        ...
        try {
            ...
            if (outProcessExitCode !== 0 && !err.isTerminated)
                throw err;
        }
    }
    finally {
