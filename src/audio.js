...
export default ({ verbose, tmpDir }) => {
        ...
        return pMap(clips, async (clip, i) => {
            ...
            async function runInner() {
                ...
                async function createSilence() {
                    if (verbose)
                        console.log("create silence", duration);
                    const args = [
                        "-nostdin",
                        "-f",
                        "lavfi",
                ...
                const audioLayers = layers.filter(({ type, start, stop }) => ["audio", "video"].includes(type) &&
                ...
                const processedAudioLayersRaw = await pMap(audioLayers, async (audioLayer, j) => {
                    ...
                    try {
                        ...
                        if (Math.abs(speedFactor - 1) > 0.01) {
                            if (verbose)
                                console.log("audio speedFactor", speedFactor);
                ...
                const processedAudioLayers = processedAudioLayersRaw.filter((r) => r !== undefined);
    ...
    async function crossFadeConcatClipAudio(clipAudio) {
        ...
        if (verbose)
            console.log("Combining audio", clipAudio.map(({ path }) => basename(path)));
        let inStream = "[0:a]";
    ...
    async function mixArbitraryAudio({ streams, audioNorm, outputVolume, }) {
        ...
        let gaussSize = 5;
        if (audioNorm) {
            if (audioNorm.gaussSize != null)
                gaussSize = audioNorm.gaussSize;
            if (audioNorm.maxGain != null)
                maxGain = audioNorm.maxGain;
        }
