...
(async () => {
    ...
    if (json) {
        ...
        const clips = await pMap(clipsIn, async (clip) => {
            ...
            if (match)
                return { type: 'title-background', text: match[1] };
            match = clip.match(/^https?:\/\/.*\.(jpg|jpeg|png|webp|gif|svg)$/); // todo improve
            if (match)
                return { type: 'image', path: clip };
    ...
    if (transitionName || transitionDuration != null) {
        ...
        if (transitionName)
            params.defaults.transition.name = transitionName;
        if (transitionDuration)
            params.defaults.transition.duration = transitionDuration;
    }
    if (clipDuration)
        params.defaults.duration = clipDuration;
    if (fontPath) {
    ...
    if (audioFilePath)
        params.audioFilePath = audioFilePath;
    if (loopAudio)
        params.loopAudio = loopAudio;
    if (outputVolume)
        params.outputVolume = outputVolume;
    if (keepSourceAudio)
        params.keepSourceAudio = true;
    if (allowRemoteRequests)
        params.allowRemoteRequests = true;
    if (width)
        params.width = width;
    if (height)
