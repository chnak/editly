...
import { readVideoFileInfo, readAudioFileInfo, assertFileValid, checkTransition, } from './util.js';
...
export default async function parseConfig({ defaults: defaultsIn = {}, clips, arbitraryAudio: arbitraryAudioIn, backgroundAudioPath, backgroundAudioVolume, loopAudio, allowRemoteRequests, ffprobePath }) {
    ...
    async function handleLayer(layer) {
        ...
        if (layer.type === 'rainbow-colors') {
            return handleLayer({ type: 'gl', fragmentPath: join(dirname, 'shaders/rainbow-colors.frag') });
        }
        ...
        if (layer.type === 'title-background') {
            ...
            if (background) {
                if (background.type === 'radial-gradient')
                    outLayers.push(await handleLayer({ type: 'radial-gradient', colors: background.colors }));
                else if (background.type === 'linear-gradient')
                    outLayers.push(await handleLayer({ type: 'linear-gradient', colors: background.colors }));
                else if (background.color)
                    outLayers.push(await handleLayer({ type: 'fill-color', color: background.color }));
            }
            else {
    ...
    let clipsOut = await pMap(clips, async (clip, clipIndex) => {
        ...
        if (firstVideoLayer && !userClipDuration)
            clipDuration = firstVideoLayer.layerDuration;
        assert(clipDuration);
